...
/* ======== Positioning / Mobility ======= */
  ...
  //AP
  positionAlloc->Add (Vector (2.0, 12.0, 1));
  ...
  //First floor in the next building
  positionAlloc->Add (Vector (32.0, 18.0, 3.0));    
  positionAlloc->Add (Vector (38.0, 18.0, 3.0));
  positionAlloc->Add (Vector (32.0, 2.0, 3.0));
  positionAlloc->Add (Vector (36.0, 8.0, 3.0));
  positionAlloc->Add (Vector (38.0, 5.0, 3.0));
  positionAlloc->Add (Vector (36.0, 2.0, 3.0));
...
/* ===== Propagation Model configuration ===== */
  ...
  Ptr<Building> c = CreateObject <Building> ();
    c->SetBoundaries (Box (30.0, 50.0, 0.0, 30.0, 0.0, 6.0));
    c->SetBuildingType (Building::Office);
    c->SetExtWallsType (Building::ConcreteWithWindows);
    c->SetNFloors (floors);
    c->SetNRoomsX (roomsInAxis);
    c->SetNRoomsY (roomsInAxis);
